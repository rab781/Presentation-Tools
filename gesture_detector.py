...
class GestureDetector:
    ...
    def detect_gesture(self, frame: np.ndarray, draw_landmarks=True) -> Tuple[Optional[str], np.ndarray]:
        ...
        # cv2.cvtColor processing time scales with the number of pixels, and reducing the
        # resolution first significantly decreases the number of pixels it must process,
        ...
        if not hasattr(self, 'color_small_buffer') or self.color_small_buffer.shape[:2] != (proc_h, proc_w):
            ...
            self.color_small_buffer = np.empty(shape, dtype=getattr(frame, 'dtype', np.uint8))
