...
class GestureDetector:
    ...
    def detect_gesture(self, frame: np.ndarray, draw_landmarks=True) -> Tuple[Optional[str], np.ndarray]:
        ...
        # In computer vision pipelines that downscale multi-channel color frames,
        # resizing (`cv2.resize`) *before* converting to grayscale (`cv2.cvtColor`) is faster.
        # `cv2.cvtColor` processing time scales heavily with the number of pixels, so reducing
        # the resolution first significantly decreases the number of pixels it must process,
        # outweighing the slightly higher cost of resizing 3 channels vs 1 channel.
        ...
        if not hasattr(self, 'color_small_buffer') or self.color_small_buffer.shape[:2] != (proc_h, proc_w):
            ...
            self.color_small_buffer = np.empty((proc_h, proc_w, channels), dtype=getattr(frame, 'dtype', np.uint8))
        ...
        # Convert the downscaled color image to grayscale
