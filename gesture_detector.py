...
class GestureDetector:
    ...
    def detect_gesture(self, frame: np.ndarray, draw_landmarks=True) -> Tuple[Optional[str], np.ndarray]:
        ...
        # is faster because cv2.cvtColor processing time scales heavily with the number of pixels.
        # Reducing the resolution first significantly decreases the number of pixels it must process,
        ...
        if not hasattr(self, 'color_small_buffer') or self.color_small_buffer.shape[:2] != (proc_h, proc_w):
            ...
            self.color_small_buffer = np.empty((proc_h, proc_w, channels) if channels > 1 else (proc_h, proc_w), dtype=getattr(frame, 'dtype', np.uint8))
