    ...
    def detect_active_application(self) -> str:
        ...
        try:
            ...
            if not hasattr(self, '_cached_window') or self._cached_window != window or self._cached_window_title != window_title:
                ...
                # ⚡ OPTIMIZATION: Early Application Detection
                # Instead of always matching process names and window titles against
                # known application profiles, we cache the resulting `detected_app`
                # along with the window handle and title. This skips redundant if/elif
                # evaluations for every iteration of the control loop.
