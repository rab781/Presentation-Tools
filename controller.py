...
class PresentationController:
    ...
    def detect_active_application(self) -> str:
        ...
        try:
            ...
            # ⚡ OPTIMIZATION: Check the window title first to avoid the slow, blocking `psutil.Process` lookup.
            # Matching obvious keywords first significantly reduces CPU overhead and detection latency.
            # "presentation" is too generic to match here, as it could be a folder or a PDF.
            if "powerpoint" in window_title or "pptx" in window_title or ".ppt" in window_title:
                ...
                # Get process name for generic titles or specific apps like PDF viewers
                try:
                ...
                if "powerpnt.exe" in process_name or "powerpnt" in process_name:
                    ...
                    # Browser - check title for generic presentation apps
