...
class PresentationController:
    ...
    def detect_active_application(self) -> str:
        ...
        try:
            ...
            # ⚡ OPTIMIZATION: Cache process resolution and profile detection
            # Calling win32process and psutil.Process is expensive due to system calls.
            # We can cache the detected process name and final profile based on the window
            # handle and title. If the foreground window hasn't changed, we can skip the
            # slow blocking calls and the repetitive string matching rules entirely.
            if hasattr(self, '_cached_window') and self._cached_window == window and self._cached_window_title == window_title:
                return self._cached_detected_app
