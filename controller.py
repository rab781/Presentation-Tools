...
class PresentationController:
    ...
    def detect_active_application(self) -> str:
        ...
        try:
            ...
            # ⚡ OPTIMIZATION: Cache process resolution and application profile
            # Calling win32process and psutil.Process is expensive due to system calls.
            # We can cache the detected process name and final application profile based on the window handle and title.
            # If the foreground window handle and title haven't changed, the underlying
            # process and application profile are the same, allowing us to skip the slow blocking calls
            # and the string matching string rule evaluations.
