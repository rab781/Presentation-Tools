...
class VoiceRecognizer:
    ...
    def _listen_loop(self):
        ...
        while self.is_listening:
            try:
                # OPTIMIZATION: Keep microphone stream open across multiple listen calls.
                # Re-initializing the audio stream (PyAudio) on every iteration is expensive
                # and can cause latency or audio artifacts.
                with self.microphone as source:
                    while self.is_listening:
                        try:
                            # Listen for audio
                            ...
                            # No speech detected, continue without resetting stream
                            continue
                        except Exception as e:
                            # Break inner loop to reset stream on other errors
                            if self.is_listening:
