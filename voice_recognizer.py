...
class VoiceRecognizer:
    ...
    def _recognize_with_vosk(self, audio) -> Optional[str]:
        ...
        try:
            ...
            # ⚡ OPTIMIZATION: Reuse pre-initialized KaldiRecognizer
            # By reusing `self.vosk_recognizer` instead of instantiating `KaldiRecognizer`
            # on every call, we prevent the overhead of re-creating the recognizer object
            # on each audio frame processed.
            # We must reset the recognizer state before processing a new isolated audio segment.
            if hasattr(self.vosk_recognizer, "Reset"):
