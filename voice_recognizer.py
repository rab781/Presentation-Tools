...
class VoiceRecognizer:
    ...
    def _listen_loop(self):
        ...
        while self.is_listening:
            # Optimize microphone access by keeping context open
            try:
                with self.microphone as source:
                    while self.is_listening:
                        try:
                            ...
                            if self.is_listening:
                                ...
                                # Break inner loop to re-initialize stream if needed
                                break
                            time.sleep(0.1)
            except Exception as e:
                if self.is_listening:
                    print(f"Fatal error in voice recognition stream: {e}")
                    # Retry after delay to prevent rapid looping on persistent errors
                    time.sleep(1)
