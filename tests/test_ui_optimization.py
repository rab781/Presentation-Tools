...
class TestUIOptimization(unittest.TestCase):
    ...
    def test_ui_uses_bitwise_shift_instead_of_add_weighted(self, mock_controller):
        # Initialize app
        ...
        # Make the slice return roi_mock
        frame.__getitem__.return_value = roi_mock

        # Call draw UI
        app._draw_ui(frame)

        # Verify addWeighted was NOT called (we replaced it with bitwise shift)
        mock_cv2.addWeighted.assert_not_called()

        # Verify the bitwise shift was applied
