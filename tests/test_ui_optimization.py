...
class TestUIOptimization(unittest.TestCase):
    ...
    def test_dimming_uses_bitwise_shift(self, mock_controller):
        ...
        # Create a mock frame and setup mock slicing behavior
        frame = MagicMock()
        frame.shape = (480, 640, 3)
        mock_roi = MagicMock()
        frame.__getitem__.return_value = mock_roi
        # Setup in-place right shift mock
        mock_roi.__irshift__.return_value = mock_roi

        # Call draw UI
        app._draw_ui(frame)

        # Verify bitwise shift was called for dimming and addWeighted was NOT called
