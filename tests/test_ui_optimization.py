...
class TestUIOptimization(unittest.TestCase):
    ...
    def test_ui_dimming_uses_bitwise_shift(self, mock_controller):
        ...
        # Configure the frame mock to return a mock ROI when sliced
        mock_roi = MagicMock()
        frame.__getitem__.return_value = mock_roi
        # Configure the mock ROI to support the __irshift__ operation (>>=)
        mock_roi.__irshift__.return_value = mock_roi

        # Call draw UI
        app._draw_ui(frame)

        # Verify cv2.addWeighted was NOT called
        mock_cv2.addWeighted.assert_not_called()

        # Verify the bitwise shift was applied to the ROI
