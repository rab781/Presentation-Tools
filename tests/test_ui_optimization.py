...
class TestUIOptimization(unittest.TestCase):
    ...
    def test_bitwise_shift_optimization(self, mock_controller):
        # Initialize app
        app = PresentationToolApp()

        # Create a mock frame and mock ROI slice
        frame = MagicMock()
        frame.shape = (480, 640, 3)
        mock_roi = MagicMock()
        frame.__getitem__.return_value = mock_roi

        # Support in-place right shift operator (roi >>= 1)
        mock_roi.__irshift__.return_value = mock_roi

        # Call draw UI
        app._draw_ui(frame)

        # Verify addWeighted is NOT called anymore
        mock_cv2.addWeighted.assert_not_called()

        # Verify in-place right shift was used on the ROI for dimming
