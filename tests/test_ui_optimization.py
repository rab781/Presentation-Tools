...
class TestUIOptimization(unittest.TestCase):
    ...
    def test_ui_dimming_uses_bitwise_shift(self, mock_controller):
        ...
        # Create a mock frame with slice support
        frame = MagicMock()
        frame.shape = (480, 640, 3)
        mock_roi = MagicMock()
        frame.__getitem__.return_value = mock_roi
        # Setup the __irshift__ magic method to return itself (in-place operation)
        ...
        # Verify the slice was taken (frame[0:120, 0:w])
        frame.__getitem__.assert_called()

        # Verify the right shift was applied
