...
class TestUIOptimization(unittest.TestCase):
    ...
    def test_ui_dimming_uses_bitwise_shift(self, mock_controller):
        ...
        # Configure slice behavior to return a mock ROI that can track in-place operations
        mock_roi = MagicMock()
        frame.__getitem__.return_value = mock_roi
        # Mock the in-place right shift (>>=)
        mock_roi.__irshift__.return_value = mock_roi

        # Call draw UI
        app._draw_ui(frame)

        # Verify addWeighted is NOT called (optimization: use bitwise shift instead)
        mock_cv2.addWeighted.assert_not_called()

        # Verify bitwise right shift was applied in-place to the ROI (roi >>= 1)
