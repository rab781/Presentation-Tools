...
class TestUIOptimization(unittest.TestCase):
    ...
    def test_ui_dimming_uses_bitwise_shift(self, mock_controller):
        ...
        # Configure the mock to support slicing and in-place right shift
        mock_roi = MagicMock()
        ...
        # Verify that the slice was taken (frame[0:120, 0:w])
        frame.__getitem__.assert_called_once()

        # Verify that the in-place bitwise right shift (>>= 1) was applied to the ROI
