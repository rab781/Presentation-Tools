...
class TestUIOptimization(unittest.TestCase):
    ...
    def test_add_weighted_replaced_with_bitwise_shift(self, mock_controller):
        # Initialize app
        app = PresentationToolApp()

        # Create a mock frame and mock ROI
        frame = MagicMock()
        frame.shape = (480, 640, 3)
        mock_roi = MagicMock()

        # Configure the slice behavior on frame to return the mock ROI
        frame.__getitem__.return_value = mock_roi

        # Support the in-place bitwise right shift operation (roi >>= 1)
        ...
        # Verify bitwise shift was used on the ROI
