...
class TestUIOptimization(unittest.TestCase):
    ...
    def test_add_weighted_not_called_uses_shift(self, mock_controller):
        # Initialize app
        app = PresentationToolApp()

        # Create a mock frame and a mock for the slice (ROI)
        frame = MagicMock()
        frame.shape = (480, 640, 3)
        mock_roi = MagicMock()

        # Configure slice behavior and in-place right shift
        ...
        # Verify that in-place right shift (>>=) was called with 1 on the ROI
