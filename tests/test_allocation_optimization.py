...
class TestAllocationOptimization(unittest.TestCase):
    ...
    def test_double_buffering_preserves_motion_detection(self):
        ...
        # Extract the dst buffers used for cvtColor across all calls
        # (cvtColor now writes the final downscaled gray image which requires double buffering)
        cvt_calls = mock_cv2.cvtColor.call_args_list
        self.assertGreaterEqual(len(cvt_calls), 3, "Expected cv2.cvtColor to be called at least 3 times for double buffering validation")

        dst_buffers = []
        for _, kwargs in cvt_calls:
