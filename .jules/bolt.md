...
## 2025-12-26 - [Use Bitwise Shift for UI Dimming]
**Learning:** For simple semi-transparent UI dimming effects in Python/OpenCV pipelines, using NumPy in-place bitwise right shifts (e.g., `roi >>= 1` to halve pixel values for ~0.5 opacity) is computationally much faster than using `cv2.addWeighted`, effectively avoiding heavy float multiplications. Replacing `cv2.addWeighted` (e.g., 0.4 multiplier) with a bitwise right shift (equivalent to 0.5 blending) slightly alters the visual brightness, which is an acceptable compromise for the performance optimization.
**Action:** When creating simple 50% opacity backgrounds or overlays, use the in-place bitwise right shift `>>= 1` on the array slice instead of using `cv2.addWeighted` to avoid floating-point overhead.
