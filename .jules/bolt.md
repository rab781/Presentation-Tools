...
## 2025-12-25 - [Fast UI dimming with bitwise shift]
**Learning:** For simple semi-transparent UI dimming effects in Python/OpenCV pipelines, using `cv2.addWeighted` introduces heavy floating-point multiplication overhead per pixel. Using a NumPy in-place bitwise right shift (e.g., `roi >>= 1` to halve pixel values for ~0.5 opacity) is computationally much faster by avoiding floats entirely. This slightly alters the visual brightness compared to a 0.4 multiplier but is an acceptable compromise for the performance optimization.
**Action:** Replace `cv2.addWeighted` with an in-place bitwise right shift (`>>=`) for simple 50% opacity UI dimming effects to eliminate floating-point arithmetic overhead in rendering loops.
