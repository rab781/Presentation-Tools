...
## 2025-12-25 - [Optimize application detection latency]
**Learning:** In Windows active-window monitoring loops, `psutil.Process()` calls are relatively expensive as they enumerate system processes or open process handles, causing latency and CPU spikes. By promoting the already-existing "window title fallback checks" (e.g. `win32gui.GetWindowText()`) to the top of the function, the code avoids the heavy `psutil` lookup entirely in the majority of positive matching cases (e.g. when the title contains "powerpoint", "google slides").
**Action:** When detecting active applications, always evaluate fast system calls (like window title checks) before slow, blocking operations (like process name lookups) to minimize latency. Be careful with overly generic keywords like `'presentation'` which should only be matched against the title after confirming the underlying process is a web browser.
