...
**Learning:** For simple semi-transparent UI dimming effects in Python/OpenCV pipelines, using NumPy in-place bitwise right shifts (e.g., `roi >>= 1` to halve pixel values for ~0.5 opacity) is computationally much faster than using `cv2.addWeighted`, effectively avoiding heavy float multiplications. Replacing `cv2.addWeighted` (e.g., 0.4 multiplier) with a bitwise right shift slightly alters the visual brightness, which is an acceptable compromise for the performance optimization of avoiding floating-point operations.
**Action:** Use in-place bitwise right shifts (`>>=`) for simple UI dimming and semi-transparent effects instead of `cv2.addWeighted` to avoid expensive floating-point math overhead.
