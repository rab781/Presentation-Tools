...
## 2025-12-25 - [Optimize order of resize and cvtColor]
**Learning:** In computer vision pipelines like `gesture_detector.py`, applying `cv2.resize` to downscale the frame before applying `cv2.cvtColor` for grayscale conversion significantly improves performance. It reduces the total number of pixels processed during the color space conversion by a factor of the processing scale squared. However, note that a 3-channel resize is computationally heavier than a 1-channel resize. Ensure that mocked behavior in tests (like `benchmark_gesture.py`) properly propagates shape dimensions to accurately simulate multi-channel resizing costs.
**Action:** Always resize large frames *before* performing pixel-heavy operations like `cv2.cvtColor`, making sure to allocate a buffer with 3 channels instead of 1, and ensure dependent test mocks handle source image depth accurately.
