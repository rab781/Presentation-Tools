...
## 2025-12-25 - [Faster semi-transparent UI dimming with bitwise shift]
**Learning:** For simple semi-transparent UI dimming effects in Python/OpenCV pipelines, using NumPy in-place bitwise right shifts (e.g., `roi >>= 1` to halve pixel values for ~0.5 opacity) is computationally much faster than using `cv2.addWeighted`, effectively avoiding heavy float multiplications per pixel.
**Action:** Replace `cv2.addWeighted` with bitwise right shifts (`>>= 1`) for simple 50% dimming overlays where visually acceptable, saving CPU cycles.
