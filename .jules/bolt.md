...
**Learning:** For simple semi-transparent UI dimming effects in Python/OpenCV pipelines, using NumPy in-place bitwise right shifts (e.g., `roi >>= 1` to halve pixel values for ~0.5 opacity) is computationally much faster than using `cv2.addWeighted`. This avoids heavy floating-point operations entirely while achieving the desired visual effect.
**Action:** Replace `cv2.addWeighted` with bitwise operations like `>>= 1` for simple dimming effects where a slight difference in brightness is an acceptable compromise for avoiding floating-point math overhead.
