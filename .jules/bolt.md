...
## 2025-12-21 - [Cache stateful decoder instances]
**Learning:** Instantiating `KaldiRecognizer` per audio segment in `_recognize_with_vosk` creates significant overhead because it sets up the decoding graph on every `_recognize_with_vosk` call, directly decreasing latency per frame.
**Action:** Cache the `KaldiRecognizer` in the class initialization instead, reducing per-frame latency. Be careful though, `KaldiRecognizer` is stateful and must be cleared between uses, requiring a `Reset()` call before feeding it new audio data to prevent cross-utterance state bleeding.
