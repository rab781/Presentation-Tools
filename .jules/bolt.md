...
## 2025-12-25 - [Use bitwise right shift for UI dimming]
**Learning:** For simple semi-transparent UI dimming effects in Python/OpenCV pipelines, using NumPy in-place bitwise right shifts (e.g., `roi >>= 1` to halve pixel values for ~0.5 opacity) is computationally much faster than using `cv2.addWeighted`, effectively avoiding heavy float multiplications. Replacing `cv2.addWeighted` (e.g., 0.4 multiplier) with a bitwise right shift (`roi >>= 1`, equivalent to 0.5 blending) slightly alters the visual brightness, which is an acceptable compromise for the performance optimization.
**Action:** Use in-place bitwise operations like `roi >>= 1` instead of `cv2.addWeighted` when exact blend weights are not strictly required, to save CPU cycles.
