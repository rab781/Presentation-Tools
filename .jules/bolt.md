...
## 2025-12-25 - [Process Name Resolution Bottleneck]
**Learning:** `psutil.Process(process_id).name()` makes a slow blocking system call to retrieve process information. In loops or frequent checks (like `detect_active_application`), this introduces massive overhead (e.g., ~500ms for 100 iterations vs <5ms for cached).
**Action:** When repeatedly looking up process names, cache the result using the Process ID (PID) retrieved from fast APIs like `win32process.GetWindowThreadProcessId`. PIDs securely map to the same process name for their lifespan, allowing for O(1) cached lookup while dramatically reducing CPU overhead and latency. Make sure to implement a simple mechanism (like `len() > 100`) to prevent unbounded cache growth.
