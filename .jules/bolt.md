...
## 2025-12-25 - [Cache expensive blocking OS calls in loops]
**Learning:** In `controller.py`, `psutil.Process(process_id).name()` is a slow blocking system call. Resolving the active application name inside a continuous loop caused significant latency.
**Action:** When repeatedly calling expensive OS-level functions that depend on the active foreground window, cache the result and validate it against the window handle (`HWND`) and title. If the handle and title remain unchanged, reuse the cached process name to drastically reduce overhead while safely avoiding PID reuse edge cases.
