...
## 2025-12-26 - [Replace cv2.addWeighted with bitwise shift for UI dimming]
...
**Action:** When a ~0.5 alpha blend dimming effect is needed, replace `cv2.addWeighted` with `roi >>= 1` to maximize frame rate and reduce CPU load.
