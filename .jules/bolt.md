...
## 2025-12-24 - [Avoid de-optimization by preserving OpenCV logic order]
**Learning:** cv2.cvtColor is vectorized and fast, while cv2.resize uses interpolation which scales heavily with the number of channels. Swapping their order (resizing first, cvtColor second) actually de-optimizes the code because it increases the interpolation work threefold (3 channels vs 1 channel) and causes critical bugs with double-buffering references (where prev_frame gets overwritten in place leading to zeroed-out motion detection).
**Action:** Do not attempt to reorder cvtColor and resize. Always convert frames to grayscale first, then perform interpolation/resize operations.
## 2025-12-25 - [Optimize application detection by skipping psutil]
**Learning:** In `controller.py`, checking `psutil.Process(process_id).name()` on every application detection cycle creates a slow, blocking system call. Checking the window title (which is already fetched via a fast `win32gui.GetWindowText` call) first allows us to skip the expensive process lookup entirely in most common scenarios.
**Action:** Use fast path checks for window titles before falling back to expensive process lookups to significantly reduce detection latency and CPU overhead.
