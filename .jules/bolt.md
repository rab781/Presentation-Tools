...
## 2025-12-25 - [Optimize UI dimming with bitwise shift]
**Learning:** For simple semi-transparent UI dimming effects in Python/OpenCV pipelines, using NumPy in-place bitwise right shifts (e.g., `roi >>= 1` to halve pixel values for ~0.5 opacity) is computationally much faster than using `cv2.addWeighted`, effectively avoiding heavy float multiplications. Additionally, because the `roi` is a view, the in-place shift automatically mutates the original `frame`, so re-assigning it back is redundant and can be omitted.
**Action:** Replace `cv2.addWeighted` with bitwise shifts for simple 50% opacity dimming to save CPU cycles and reduce overhead, and avoid redundant array view assignments.
