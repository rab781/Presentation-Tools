...
## 2025-12-25 - [Double-buffering on cvtColor instead of resize]
**Learning:** In the image processing pipeline, applying `cv2.resize` to the color frame *before* `cv2.cvtColor` improves performance by significantly reducing the total pixel count that `cvtColor` has to process. Because of this reordering, the downscaled grayscale frame is now generated by `cvtColor`. Therefore, the double-buffering scheme intended to safely preserve the previous frame's state for motion detection without allocation must be applied to `cvtColor`, not `resize`.
**Action:** When reversing the order of downscaling and grayscale conversion, migrate the double-buffering logic to the final operation that generates the frame used for motion detection (e.g., `cvtColor` outputting the final grayscale frame), ensuring we preserve the previous buffer without accidental overwrite.
