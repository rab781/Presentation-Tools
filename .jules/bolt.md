...
## 2025-07-26 - Fast in-place UI dimming with bitwise shifts
**Learning:** For simple semi-transparent UI dimming effects in Python/OpenCV pipelines, using NumPy in-place bitwise right shifts (e.g., `roi >>= 1` to halve pixel values for ~0.5 opacity) is computationally much faster than using `cv2.addWeighted`, effectively avoiding heavy float multiplications. Replacing `cv2.addWeighted` (e.g., 0.4 multiplier) with a bitwise right shift (equivalent to 0.5 blending) slightly alters the visual brightness, which is an acceptable compromise for the performance optimization. In OpenCV/NumPy pipelines, when performing in-place operations on an array slice (`roi = frame[...]`), the original array (`frame`) is automatically mutated because the slice is a view. Re-assigning the slice back to the array is redundant.
**Action:** When adding simple dimming effects to frames in computer vision pipelines, prefer in-place bitwise shifts (`>>= 1`) over `cv2.addWeighted` when exact opacity multipliers (like 0.4) are not strictly necessary and performance is critical.
