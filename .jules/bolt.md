...
**Learning:** For simple semi-transparent UI dimming effects in Python/OpenCV pipelines, using NumPy in-place bitwise right shifts (e.g., `roi >>= 1` to halve pixel values for ~0.5 opacity) is computationally much faster than using `cv2.addWeighted`, effectively avoiding heavy floating-point multiplications.
**Action:** Replace `cv2.addWeighted` with bitwise right shifts when possible to avoid expensive floating-point operations in UI rendering hot loops.
