...
## 2025-12-26 - [Replace addWeighted with Bitwise Shift for UI Dimming]
**Learning:** For simple semi-transparent UI dimming effects in Python/OpenCV pipelines, using NumPy in-place bitwise right shifts (e.g., `roi >>= 1` to halve pixel values for ~0.5 opacity) is computationally much faster than using `cv2.addWeighted`, effectively avoiding heavy float multiplications. This slightly alters the visual brightness, which is an acceptable compromise for the performance optimization.
**Action:** Replace `cv2.addWeighted` with bitwise right shifts when simple halving of pixel values is sufficient for visual effects.
