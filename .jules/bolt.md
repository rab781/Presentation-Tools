...
**Learning:** For simple semi-transparent UI dimming effects in Python/OpenCV pipelines, using `cv2.addWeighted` introduces floating-point multiplication overhead. Using NumPy in-place bitwise right shifts (e.g., `roi >>= 1` to halve pixel values for ~0.5 opacity) is computationally much faster while modifying the original array view without allocating new arrays.
**Action:** Replace `cv2.addWeighted` with bitwise right shifting (`>>= 1`) for simple UI dimming overlays to avoid floating-point math overhead.
