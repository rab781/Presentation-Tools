...
class PresentationToolApp:
    ...
    def _draw_ui(self, frame: np.ndarray) -> np.ndarray:
        ...
        roi = frame[0:120, 0:w]
        # ⚡ OPTIMIZATION: Bitwise right shift for UI dimming
        # Replaced cv2.addWeighted with in-place bitwise right shift (roi >>= 1).
        # This acts as a fast 0.5 alpha blend by halving pixel values, avoiding
        # expensive floating-point multiplications. Since 'roi' is a view into 'frame',
        # the in-place operation directly mutates the original frame without reassignment.
        # Expected Impact: Significantly reduces UI drawing overhead and avoids float math.
        roi >>= 1
