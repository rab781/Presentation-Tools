...
class PresentationToolApp:
    ...
    def _draw_ui(self, frame: np.ndarray) -> np.ndarray:
        ...
        roi = frame[0:120, 0:w]
        # ⚡ OPTIMIZATION: Bitwise shift for fast UI dimming
        # Using an in-place bitwise right shift (equivalent to halving values) is
        # significantly faster than `cv2.addWeighted` for simple dimming effects because
        # it avoids floating-point operations. Since `roi` is a view of `frame`, this
        # modifies the original array in-place, removing the need for a re-assignment.
        roi >>= 1
