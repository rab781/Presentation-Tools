...
class PresentationToolApp:
    ...
    def _draw_ui(self, frame: np.ndarray) -> np.ndarray:
        ...
        roi = frame[0:120, 0:w]
        # ⚡ OPTIMIZATION: Bitwise shift for fast dimming
        # For simple semi-transparent UI dimming effects, using an in-place bitwise
        # right shift (roi >>= 1) halves the pixel values, effectively creating ~0.5 opacity.
        # This is computationally much faster than `cv2.addWeighted` as it avoids
        # heavy floating point multiplications per pixel while achieving a visually similar effect.
        roi >>= 1
