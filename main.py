...
class PresentationToolApp:
    ...
    def _draw_ui(self, frame: np.ndarray) -> np.ndarray:
        ...
        # ⚡ OPTIMIZATION: Faster UI dimming with bitwise shift
        # For simple semi-transparent UI dimming effects, using NumPy in-place
        # bitwise right shifts (e.g., `roi >>= 1` to halve pixel values for ~0.5 opacity)
        # is computationally much faster than using `cv2.addWeighted`, effectively
        # avoiding heavy float multiplications. This directly mutates the `frame` array slice.
        roi = frame[0:120, 0:w]
        roi >>= 1
