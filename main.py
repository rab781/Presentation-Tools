...
class PresentationToolApp:
    ...
    def _draw_ui(self, frame: np.ndarray) -> np.ndarray:
        ...
        # Instead of using `cv2.addWeighted` for alpha blending (which involves
        # heavy floating point multiplications per pixel), we use an in-place
        # bitwise right shift (`>>= 1`) on the Region of Interest (ROI).
        # This divides pixel values by 2 (effectively 0.5 opacity) using fast
        # integer math, eliminating float overhead while maintaining the visual dimming effect.
        # Since the numpy slice `roi` is a view into `frame`, the in-place shift
        # mutates the original frame automatically without needing re-assignment.
        # Expected Impact: Eliminates expensive float multiplications, reducing UI rendering time.
        roi = frame[0:120, 0:w]
        roi >>= 1
