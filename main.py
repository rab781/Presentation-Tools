...
class PresentationToolApp:
    ...
    def _draw_ui(self, frame: np.ndarray) -> np.ndarray:
        ...
        # ⚡ OPTIMIZATION: Fast UI Dimming via Bitwise Shift
        # Instead of copying the entire frame or using the computationally expensive
        # cv2.addWeighted (which involves floating-point multiplications), we extract
        # the Region of Interest (ROI) for the top banner and use an in-place bitwise
        # right shift (>>= 1). This effectively halves the pixel values (equivalent to
        # 0.5 opacity) using integer operations, bypassing heavy float multiplications.
        # Furthermore, since `roi` is a view of `frame`, mutating it in-place
        # automatically updates `frame`, eliminating the need to re-assign the slice.
        # Expected Impact: Significantly faster blending computation with zero allocations.
        roi = frame[0:120, 0:w]
        roi >>= 1
