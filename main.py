...
class PresentationToolApp:
    ...
    def _draw_ui(self, frame: np.ndarray) -> np.ndarray:
        ...
        # Using a NumPy in-place bitwise right shift (`roi >>= 1`) to halve pixel values
        # is computationally much faster than `cv2.addWeighted` for simple semi-transparent effects,
        # effectively avoiding heavy float multiplications. This perfectly achieves an ~0.5 opacity effect.
        roi >>= 1
