...
class PresentationToolApp:
    ...
    def _draw_ui(self, frame: np.ndarray) -> np.ndarray:
        ...
        # For simple semi-transparent UI dimming, using NumPy in-place bitwise right
        # shifts (roi >>= 1) to halve pixel values (approx alpha=0.5) is computationally
        # much faster than using cv2.addWeighted with floating point multiplications.
        roi >>= 1
