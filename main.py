...
class PresentationToolApp:
    ...
    def _draw_ui(self, frame: np.ndarray) -> np.ndarray:
        ...
        # For simple semi-transparent UI backgrounds, multiplying by floats
        # using cv2.addWeighted is computationally expensive.
        # Halving the pixel values using an in-place bitwise right shift (>>= 1)
        # achieves a ~0.5 alpha blend much faster and entirely avoids floating point math.
        # Since 'roi' is a view into the original array slice, the in-place operation
        # automatically mutates 'frame' without requiring re-assignment.
        roi >>= 1
