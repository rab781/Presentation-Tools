...
class PresentationToolApp:
    ...
    def _draw_ui(self, frame: np.ndarray) -> np.ndarray:
        ...
        # We use an in-place bitwise right shift instead of cv2.addWeighted for
        # semi-transparent dimming to avoid expensive float multiplications.
        roi = frame[0:120, 0:w]
        # ⚡ OPTIMIZATION: Fast in-place UI dimming using bitwise operations
        # Using a bitwise right shift (equivalent to halving pixel values for ~0.5 opacity)
        # on the Region of Interest (ROI) is computationally much faster than cv2.addWeighted.
        # Since the ROI is a view into the original frame array, performing this in-place
        # (roi >>= 1) directly mutates the original frame without requiring re-assignment,
        # avoiding both floating point arithmetic and redundant array assignment overhead.
        roi >>= 1
