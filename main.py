...
class PresentationToolApp:
    ...
    def _draw_ui(self, frame: np.ndarray) -> np.ndarray:
        ...
        roi = frame[0:120, 0:w]
        # ⚡ OPTIMIZATION: Use in-place bitwise right shift instead of cv2.addWeighted
        # Using a bitwise right shift (equivalent to halving pixel values, or ~0.5 opacity)
        # completely avoids floating point multiplications involved in cv2.addWeighted.
        # Since 'roi' is a NumPy view of the original 'frame' array, the in-place operation
        # (>>=) directly mutates the original frame memory. Reassigning the slice back
        # to the array (frame[0:120, 0:w] = roi) is redundant and omitted.
        roi >>= 1
