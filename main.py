...
class PresentationToolApp:
    ...
    def _draw_ui(self, frame: np.ndarray) -> np.ndarray:
        ...
        # For simple semi-transparent UI dimming, an in-place bitwise right shift
        # (roi >>= 1) halves the pixel values, approximating a 0.5 opacity blend.
        # This is computationally much faster than cv2.addWeighted, avoiding heavy
        # float multiplications. The slice view means it mutates the original frame.
        # Expected Impact: Significant reduction in CPU time for UI rendering.
        roi >>= 1
