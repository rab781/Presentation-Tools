...
class PresentationToolApp:
    ...
    def _draw_ui(self, frame: np.ndarray) -> np.ndarray:
        ...
        # Expected Impact: Eliminates one full frame allocation.
        roi = frame[0:120, 0:w]
        # ⚡ OPTIMIZATION: Bitwise right shift for UI dimming
        # Replacing cv2.addWeighted (e.g., 0.4 multiplier) with an in-place bitwise
        # right shift (roi >>= 1, equivalent to 0.5 blending) completely avoids
        # heavy floating-point operations. The slice is a view, so mutating it
        # in-place automatically updates the original frame array.
        roi >>= 1
