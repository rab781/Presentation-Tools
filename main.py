...
class PresentationToolApp:
    ...
    def _draw_ui(self, frame: np.ndarray) -> np.ndarray:
        ...
        # For simple semi-transparent UI dimming effects (~0.5 opacity), using a NumPy in-place
        # bitwise right shift (`roi >>= 1`) is significantly faster than using `cv2.addWeighted`,
        # as it effectively avoids heavy float multiplications and operates directly on the memory.
        roi >>= 1
