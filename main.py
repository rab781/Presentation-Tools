...
class PresentationToolApp:
    ...
    def _draw_ui(self, frame: np.ndarray) -> np.ndarray:
        ...
        # ⚡ OPTIMIZATION: Use bitwise right shift for UI dimming
        # Instead of using cv2.addWeighted which performs expensive floating-point
        # operations, we use an in-place bitwise right shift (roi >>= 1).
        # This halves the pixel values, creating an equivalent ~0.5 opacity dimming effect
        # much faster. Since roi is a view of the numpy array, the original frame
        # is automatically mutated, avoiding reassignment.
        roi = frame[0:120, 0:w]
        roi >>= 1
