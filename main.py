...
class PresentationToolApp:
    ...
    def _draw_ui(self, frame: np.ndarray) -> np.ndarray:
        ...
        roi = frame[0:120, 0:w]
        # ⚡ OPTIMIZATION: Bitwise shift instead of floating-point arithmetic
        # For simple semi-transparent UI dimming effects, using an in-place bitwise
        # right shift (roi >>= 1) halves the pixel values, effectively applying a
        # 0.5 opacity blend. This avoids the heavy floating point multiplications
        # and memory allocations of cv2.addWeighted.
        roi >>= 1
