...
class PresentationToolApp:
    ...
    def _draw_ui(self, frame: np.ndarray) -> np.ndarray:
        ...
        roi = frame[0:120, 0:w]
        # ⚡ OPTIMIZATION: In-place bitwise shifting instead of addWeighted
        # For simple semi-transparent UI dimming effects, using a NumPy in-place
        # bitwise right shift (roi >>= 1) halves pixel values (~0.5 opacity) and is
        # computationally much faster than cv2.addWeighted, avoiding heavy float
