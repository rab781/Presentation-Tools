...
class PresentationToolApp:
    ...
    def _draw_ui(self, frame: np.ndarray) -> np.ndarray:
        ...
        roi = frame[0:120, 0:w]
        # ⚡ OPTIMIZATION: Bitwise shift for semi-transparent UI dimming
        # For simple semi-transparent UI dimming effects in Python/OpenCV pipelines,
        # using NumPy in-place bitwise right shifts (e.g., `roi >>= 1` to halve pixel values
        # for ~0.5 opacity) is computationally much faster than using `cv2.addWeighted`,
        # effectively avoiding heavy float multiplications.
        roi >>= 1
