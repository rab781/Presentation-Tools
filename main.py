...
class PresentationToolApp:
    ...
    def _draw_ui(self, frame: np.ndarray) -> np.ndarray:
        ...
        roi = frame[0:120, 0:w]
        # ⚡ OPTIMIZATION: Fast in-place UI dimming
        # Replaced cv2.addWeighted with in-place bitwise right shift (roi >>= 1).
        # This roughly halves the pixel values (equivalent to ~0.5 opacity) and is
        # computationally much faster by avoiding heavy floating-point multiplications.
        # Since roi is a view of frame, this in-place operation directly mutates the frame.
        roi >>= 1
