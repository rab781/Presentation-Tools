...
class PresentationToolApp:
    ...
    def _draw_ui(self, frame: np.ndarray) -> np.ndarray:
        ...
        # ⚡ OPTIMIZATION: Simple bitwise right shift for semi-transparent UI dimming
        # Instead of using `cv2.addWeighted` for alpha blending (which involves heavy
        # float multiplications per pixel), we use an in-place bitwise right shift
        # (`>>= 1`) on the Region of Interest (ROI) to quickly halve pixel values.
        # This creates a ~0.5 opacity darkening effect while being significantly faster
        # and avoiding expensive full-frame memory allocations or floating point math.
        # Expected Impact: Noticeable CPU usage reduction during UI rendering.
        roi = frame[0:120, 0:w]
        roi >>= 1
