...
class PresentationToolApp:
    ...
    def _draw_ui(self, frame: np.ndarray) -> np.ndarray:
        ...
        # For simple semi-transparent UI dimming effects in Python/OpenCV pipelines,
        # using NumPy in-place bitwise right shifts (e.g., `roi >>= 1` to halve pixel
        # values for ~0.5 opacity) is computationally much faster than using `cv2.addWeighted`,
        # effectively avoiding heavy float multiplications. Since `roi` is a view,
        # `roi >>= 1` modifies the original `frame` array in place, making re-assignment redundant.
        roi >>= 1
