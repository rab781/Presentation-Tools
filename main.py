...
class PresentationToolApp:
    ...
    def _draw_ui(self, frame: np.ndarray) -> np.ndarray:
        ...
        # For simple semi-transparent UI dimming effects, using an in-place
        # bitwise right shift (`roi >>= 1`) halves the pixel values (equivalent to ~0.5 opacity).
        # This is computationally much faster than `cv2.addWeighted` because it avoids
        # heavy floating-point multiplications entirely, while still achieving the desired visual effect.
        # Since `roi` is a slice view of `frame`, this in-place operation directly mutates `frame`.
        roi >>= 1
