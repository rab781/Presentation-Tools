...
class PresentationToolApp:
    ...
    def _draw_ui(self, frame: np.ndarray) -> np.ndarray:
        ...
        # Replacing cv2.addWeighted (which does floating point math for alpha blending)
        # with an in-place bitwise right shift (`roi >>= 1`) halves pixel values directly.
        # This acts as a ~0.5 opacity dimming effect but is computationally much faster
        # because it avoids all floating-point multiplications per pixel.
        roi >>= 1
