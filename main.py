...
class PresentationToolApp:
    ...
    def _draw_ui(self, frame: np.ndarray) -> np.ndarray:
        ...
        # Replace expensive floating-point `cv2.addWeighted` blending with an in-place
        # bitwise right shift (`>>= 1`), which halves the pixel values (~0.5 opacity).
        # This completely avoids floating-point multiplications and is significantly faster.
        # Because `roi` is a view of `frame`, in-place operations mutate `frame` automatically.
        roi >>= 1
