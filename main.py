...
class PresentationToolApp:
    ...
    def _draw_ui(self, frame: np.ndarray) -> np.ndarray:
        ...
        # ⚡ OPTIMIZATION: Fast integer arithmetic for dimming
        # Instead of using cv2.addWeighted which requires expensive floating point multiplications
        # across 3 channels for every pixel, we use an in-place bitwise right shift (>>= 1).
        # This halves the pixel values (equivalent to ~0.5 opacity blending) but is significantly
        # faster because it uses raw integer operations.
        # Because `roi` is a view of the `frame` slice, the in-place operation mutates the original
        # array automatically, avoiding any need to re-assign the slice.
        roi >>= 1
