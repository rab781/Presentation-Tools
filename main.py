...
class PresentationToolApp:
    ...
    def _draw_ui(self, frame: np.ndarray) -> np.ndarray:
        ...
        # reduces the area processed by the blending operation, saving CPU cycles and
        ...
        roi = frame[0:120, 0:w]
        # ⚡ OPTIMIZATION: In-place alpha blending using bitwise operations
        # Using a bitwise right shift (`roi >>= 1`) to halve pixel values for ~0.5 opacity
        # is computationally much faster than using `cv2.addWeighted`, avoiding heavy
        # float multiplications. Because `roi` is a view into `frame`, this in-place
        # operation automatically updates `frame` without additional memory allocations.
        roi >>= 1
