...
class PresentationToolApp:
    ...
    def _draw_ui(self, frame: np.ndarray) -> np.ndarray:
        ...
        # ⚡ OPTIMIZATION: Faster bitwise shift for UI dimming
        # Instead of using cv2.addWeighted (which performs floating-point math),
        # we can use an in-place bitwise right shift (`>>= 1`) on the array.
        # This divides pixel values by 2 (equivalent to 0.5 blending), which is
        # visually similar for a dimming effect but computationally much faster.
        # Because `roi` is a view into `frame`, the modification is automatically
        # applied to the original array and no re-assignment is needed.
        roi = frame[0:120, 0:w]
        roi >>= 1
