...
class PresentationToolApp:
    ...
    def _draw_ui(self, frame: np.ndarray) -> np.ndarray:
        ...
        # ⚡ OPTIMIZATION: Semi-transparent UI dimming
        # Instead of using cv2.addWeighted to dim the top banner, we use an in-place
        # bitwise right shift (>>= 1) on the Region of Interest (ROI). This effectively
        # halves the pixel values, creating an ~0.5 opacity dimming effect.
        # Expected Impact: Computationally much faster than cv2.addWeighted (avoiding heavy float multiplications)
        # while keeping the operation fully in-place on the existing frame slice.
        roi = frame[0:120, 0:w]
        roi >>= 1
