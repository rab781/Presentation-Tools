...
class PresentationToolApp:
    ...
    def _draw_ui(self, frame: np.ndarray) -> np.ndarray:
        ...
        roi = frame[0:120, 0:w]
        # ⚡ OPTIMIZATION: Bitwise shift for alpha blending
        # Replacing cv2.addWeighted with a bitwise right shift (`roi >>= 1`) halves the pixel
        # values, effectively creating a 50% opacity dimming effect. This avoids expensive
        # floating-point arithmetic used in cv2.addWeighted and is significantly faster.
        # Since `roi` is a view of `frame`, modifying it in-place also directly modifies the frame,
        # removing the need for a redundant assignment back to `frame[0:120, 0:w]`.
        roi >>= 1
