...
class PresentationToolApp:
    ...
    def _draw_ui(self, frame: np.ndarray) -> np.ndarray:
        ...
        roi = frame[0:120, 0:w]
        # ⚡ OPTIMIZATION: In-place bitwise shifting for simple UI dimming
        # Using an in-place bitwise right shift (roi >>= 1) halves pixel values (~0.5 opacity)
        # without requiring heavy floating-point operations from cv2.addWeighted.
        # Since roi is a view of frame, this operation safely mutates the original frame.
        roi >>= 1
