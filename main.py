...
class PresentationToolApp:
    ...
    def _draw_ui(self, frame: np.ndarray) -> np.ndarray:
        ...
        # We also use bitwise shift instead of cv2.addWeighted for dimming.
        roi = frame[0:120, 0:w]
        # ⚡ OPTIMIZATION: In-place bitwise right shifting
        # For simple semi-transparent UI dimming effects, using NumPy in-place bitwise
        # right shifts (`roi >>= 1` to halve pixel values for ~0.5 opacity) is computationally
        # much faster than using `cv2.addWeighted`, effectively avoiding heavy float multiplications.
        roi >>= 1
